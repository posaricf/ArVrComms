...
public class LeftHandSync : MonoBehaviourPun, IPunObservable
{
    ...
    private void Start()
    {
        ...
        if (isUser)
        {
            ...
            if (photonView.IsMine) GenericNetworkManager.Instance.leftHandUser = photonView;
        }

        GameObject mrRig = GameObject.FindGameObjectWithTag("MRTK Rig");

        leftHandRig = mrRig.transform.Find("Camera Offset/MRTK LeftHand Controller");
        ...
        int index = 0;
        Transform modelParent = leftHandRig.GetChild(5).GetChild(0).GetChild(1);
    ...
    private void Update()
    {
        ...
        {
            MapPosition(_leftHand, leftHandRig);
            for (int i = 0; i < _bones.Count; i++)
            {
